...
def per_sentence_dict(sentence):
    ...
    for word in sentence:
        ...
        for word2 in sentence:
            if word == word2:
                pass
            elif not word2 in d_main[word].items():
                d_perword[word2]=1
            else:
                d_perword[word2]+=1
        if d_main[word] == {}:
...
def run_similarity_test(filename, semantic_descriptors, similarity_fn):
    ...
    for questions in range(len(split_ans)):
        ...
        comp_ans = most_similar_word(choices[0], choices, semantic_descriptors, similarity_fn)
...
#print(run_similarity_test("message2.txt", some_texts, cosine_similarity))
